...
Bereits umgesetzt: PDF-Quellenimport, Extraktion (Anbieter/Prüfungscode/Version/Domains/Objectives), Aufteilung in referenzierbare `source_chunks`, editierbarer Blueprint-Entwurf mit Validierungsfehlern/-warnungen, manuelle Bestätigung niedriger Extraktionssicherheit je Objective (`blueprintDrafts.confirmedLowConfidenceObjectives`), getrennte Freigabe-/Entwurfsstatus, Quellen-/Modell-/Prompt-Version je Lesson, Fragen/Lessons mit Quellen verknüpft, Versionsvergleich (`.../sources/[id]/diff`), veraltete Inhalte werden markiert (`questions.stale`) statt gelöscht, alte Lernverläufe bleiben lesbar. URL-Quellen ergänzt: `registerCertificationSourceFromUrl()` (`lib/server/admin/sources.ts`) ruft die angegebene URL serverseitig ab und speichert die Bytes identisch zu einem PDF-Upload (Checksum-Dedup, lokale Ablage, dieselbe Extraktions-/Review-Pipeline) - PDF- und HTML-Inhalt werden anhand des Content-Type erkannt, HTML grob zu Text bereinigt (`stripHtml()`, dieselbe Logik wie im Blueprint-Preflight aus `course_generation.md`). SSRF-abgesichert über `lib/server/network/ssrf-guard.ts` (`net.BlockList`-basiert, blockt private/reservierte IPv4-/IPv6-Bereiche inkl. Cloud-Metadata-Adressen, prüft alle DNS-Antworten, folgt keinen Redirects).

Noch offen: eine von der Objective-Konfidenz unabhängige Prüfung auf einzelne unbelegte Aussagen/Behauptungen innerhalb einer Lesson.
...
Bereits umgesetzt: Zeitbudget als primäre Session-Eingabe (`StartSessionPicker`: 2/5/10/20-Minuten-Buttons auf der Kursseite, sichtbar solange noch keine gültige Session für heute existiert; `startStudySession()` baut die Session explizit mit dem gewählten Budget, OHNE das langfristige Tagesziel in `study_profiles` stillschweigend zu überschreiben), „Nur eine Aufgabe“-Modus (fünfter Button in `StartSessionPicker`, nutzt denselben Mechanismus mit `goalType: "questions", goalValue: 1` - keine eigene Session-Builder-Logik nötig, da `estimateTargetQuestionCount()` das bereits auf eine Zielgröße von genau 1 Frage abbildet), optionaler Energiezustand (drei Buttons „Wenig Energie/Geht so/Fit“ unter den Zeitbudget-Buttons, rein lokaler Auswahlzustand ohne eigene Bestätigung - `low` lässt `reallocateTowardFamiliarContent()` neuen Stoff zugunsten von Wiederholung/Schwachstellen zurückschieben, auf `study_sessions.energyLevel` gespeichert), sanfter Comeback-Modus nach Pause (`isComebackSession()`: ab 3 Tagen seit der letzten abgeschlossenen/ausgesetzten Session automatisch erkannt, ohne Nutzereingabe - dieselbe Zurückschiebung wie beim Energiezustand, dazu eine Begrüßung im Dashboard „Schön, dass du wieder da bist!“, Ergebnis als `study_sessions.isComeback`-Snapshot gespeichert, damit die Begründungs-UI auch später noch erklären kann warum), Prüfungstermin/Lerntage, Session unterbrechen/fortsetzen (`study_sessions` planned/in_progress), SM-2-artiger Review-Scheduler (`lib/server/review/scheduler.ts`, voll unit-getestet), Priorisierung schwacher/prüfungsrelevanter Bereiche und Rückstau-Berücksichtigung (`computeSessionComposition`, `reallocateTowardFamiliarContent()` - ursprünglich nur für Prüfungsdringlichkeit, jetzt derselbe Hebel für alle drei Auslöser Prüfungstermin/Energie/Comeback), nicht-strafende Serienzählung, deterministische Session-Zusammenstellung ohne KI-Aufruf, Selbsteinschätzung der Antwortsicherheit (`quizAnswers.confidence`/`studySessionItems.confidence`, dreistufig geraten/unsicher/sicher - VOR Aufdeckung der Korrektheit abgefragt, siehe `QuizQuestionCard`/`StudySession`, auch offline über die Sync-Queue erfasst), Begründungstext direkt an der Session-Empfehlung (`getSessionCategoryBreakdown()` zählt die bereits gebauten `study_session_items` je Kategorie, `TodayDashboard` baut daraus einen lesbaren Satz, z. B. "Enthält 3 fällige Wiederholungen und 1 neue Frage" - reine Textualisierung der ohnehin schon vom Scheduler getroffenen Entscheidung, keine neue Logik), Nutzerfeedback auf Empfehlungen (Daumen-hoch/-runter je Session direkt neben dem Begründungstext, `session_recommendation_feedback` mit Unique-Constraint auf `session_id` - ein zweiter Klick überschreibt die Bewertung per Upsert statt einen weiteren Eintrag anzulegen, `POST /api/study-sessions/[sessionId]/feedback`).

Noch offen: ein Hinweis-/Hint-System samt Protokollierung; Antwortgeschwindigkeit als Signal. Diese Lücken hängen eng mit dem neuen Skill-/Mission-Modell (R7) zusammen und werden dort mit aufgegriffen, sollen aber, wo sinnvoll, schon vorher in die bestehende Session ergänzt werden. Die erfasste Antwortsicherheit wird bewusst noch NICHT in den SM-2-Scheduler eingespeist (Kalibrierung zwischen Sicherheit und Korrektheit ist laut Abschnitt 29.1 eine eigene Lernwirkungs-Kennzahl, kein Scheduler-Eingang) - das bleibt ein Adaptionssignal für R7. Ebenso wird der erfasste Energiezustand bewusst noch NICHT über die aktuelle Session hinaus gespeichert/ausgewertet (kein Trend, keine Vorschläge basierend auf früheren Angaben) - reines Eingangssignal für den Session Builder, kein Lernprofil-Merkmal.
...
Bereits umgesetzt: "Inhalte als falsch, unklar oder veraltet melden" + Review-Queue (neue `content_reports`-Tabelle, `targetType`/`targetId` ohne DB-FK nach demselben Muster wie `auditEvents`/`studySessionItems` - Lerner melden eine Frage mit einem von drei Gründen direkt an der aufgedeckten Antwort, sowohl in `QuizQuestionCard` (Section-Quiz/Remediation) als auch in `StudySession` (die tägliche Session, wo Korrektheit erst am Ende gezeigt wird - Meldung daher immer sichtbar, nicht erst nach Aufdeckung); Admin sieht offene Meldungen je Zertifizierung in einer echten Queue (`ContentReportQueue`, ersetzt eine zuvor rein statische Infobox) und schließt sie als "erledigt" oder "verworfen" ab, protokolliert im Audit-Log. "Auffällige Fragen markieren" ist bereits als automatische Analytik vorhanden (`contentAnalytics.problemQuestions` - niedrige Erfolgsquote), nicht als manuelles Markieren. Health-Endpunkt (`/api/health`, von `docker-compose.yml`s Healthcheck genutzt) existiert bereits aus R0.3/R4. Readiness-Endpunkt (`/api/ready`) ergänzt: prüft zusätzlich zum reinen Prozess-Start die tatsächliche Abhängigkeit (`SELECT 1` gegen die DB) und antwortet mit 503, wenn diese nicht erreichbar ist - bewusst getrennt von `/api/health` (Liveness) statt es zu ersetzen, da der docker-compose-Healthcheck weiterhin nur den Prozessstart braucht, nicht eine harte DB-Abhängigkeit direkt beim Cloudflared-Start-Gate. "Modell, Tokenverbrauch und geschätzte Kosten pro Job" (`content_generation_jobs.model`/`promptTokens`/`completionTokens`/`totalTokens`/`estimatedCostUsd`, live mitgeschrieben über jede `USAGE`-Zeile, die `requestJson()` in `lib/ai/generate.ts` für JEDEN Antwortversuch protokolliert - auch verworfene, da vom Anbieter trotzdem abgerechnet; überlebt damit auch einen Absturz mitten im Job, da die Kindprozess-Skripte per stdout mit dem Job-Tracking kommunizieren, siehe `runNpmScript()`. `estimatedCostUsd` bleibt bewusst `NULL`, solange für das jeweilige Modell keine Preise konfiguriert sind (`ANTHROPIC_CURRICULUM_PRICE_PER_MILLION_*_TOKENS_USD` / `ANTHROPIC_LESSONS_PRICE_PER_MILLION_*_TOKENS_USD`), statt eine erfundene Zahl anzuzeigen). "Budgetgrenzen" + "vorhandene Inhalte auch bei erreichtem KI-Budget weiter ausliefern" (`getAiBudgetStatus()` summiert `estimatedCostUsd` über ALLE Jobs, auch fehlgeschlagene, da der Anbieter pro Aufruf abrechnet, nicht pro Erfolg; ist die konfigurierbare Grenze `ANTHROPIC_BUDGET_USD` erreicht, blockt die Generierungs-API neue Jobs mit HTTP 402 und die Admin-UI zeigt den Verbrauch an und deaktiviert den Start-Button - bereits generierte/veröffentlichte Inhalte bleiben unberührt, da Auslieferung und Generierung ohnehin getrennte Pfade sind; NULL/keine Grenze, solange `ANTHROPIC_BUDGET_USD` nicht gesetzt ist). KI-Anbindung war zwischenzeitlich von Gemini auf die Anthropic Claude API umgestellt (`lib/claude.ts`, ersetzte `lib/gemini.ts`): Modell wurde ein expliziter Parameter von `requestJson()`/`generateStructured()` statt eines global fixen Modells, strukturierte Ausgabe lief über Claudes natives `output_config.format`. Da ein einzelner Job zwei unterschiedlich bepreiste Modelle durchlaufen kann (Curriculum- und Lessons-Phase), rechnet `estimateCostUsd()` pro USAGE-Zeile weiterhin mit dem Preis des jeweils genutzten Modells statt einmalig mit einem globalen Preis auf die kumulierten Tokens - das blieb beim folgenden Wechsel unverändert. **Update 24. September 2026:** Der Nutzer hat die KI-Anbindung bewusst zurück auf Google Gemini umgestellt (`lib/claude.ts` nutzt jetzt `GoogleGenAI` statt des Anthropic-SDK, Default-Modell `gemini-2.5-flash`, Env-Var `GEMINI_API_KEY` statt `ANTHROPIC_API_KEY`), um ein anderes Modell zu testen - Details und die dabei live gefundenen/gefixten Bugs stehen im "Umsetzungsstand" von `course_generation.md`.

**Update 24. September 2026:** "Kosten je veröffentlichter Lesson, Mission und akzeptierter Frage" ergänzt - neue Tabelle `objective_generation_costs` (Snapshot je Objective statt je Job, da das generierende Skript keinen zuverlässigen Job-Kontext hat, siehe Kommentar auf der Tabelle in `lib/server/db/schema.ts`), befüllt direkt von `scripts/generate-lessons-and-questions.ts` über den bereits vorhandenen, bisher ungenutzten `onUsage`-Callback von `generateStructured()`. Ein Kosten-je-Element-Wert wird beim Lesen abgeleitet (Batch-Kosten geteilt durch akzeptierte Lessons+Fragen), NICHT als erfundene Einzelzahl gespeichert - Batch-Calls erzeugen mehrere Elemente gemeinsam, echte Einzelkosten sind nicht separat messbar. Sichtbar im Admin-Bereich der jeweiligen Zertifizierung ("Kosten je Objective").

Kosten je Objective des neuen One-Click-Blueprint-Pfads (`course_generation.md`) ebenfalls sichtbar gemacht: `courseGenerationObjectives.costUsd`/`inputTokens`/`outputTokens` wurden dort bereits geschrieben, aber nirgendwo angezeigt - `CourseGenerationPanel.tsx` zeigt sie jetzt in einer ausklappbaren Detailtabelle je Objective (kein Backend-Change nötig, die Daten waren schon da).

Noch offen: strukturierte Logs mit Request-/Job-ID, API-/DB-/Auth-/Jobmetriken; Änderungshistorie und erneute Freigabe nach relevanten Änderungen (aktuell nur `LessonReviewStatus`/`stale`-Markierung bei neuer Quellenversion, siehe R1, keine allgemeine Content-Versionshistorie); die gesamte Lernwirkungs-Kategorie (Behaltensleistung nach 7/30/90 Tagen, Transferleistung, benötigte Hinweise - setzt das noch offene Hint-System aus R2 voraus, Verbesserung wiederkehrender Fehler, Abbruchgründe, Qualität adaptiver Empfehlungen über das reine Daumen-hoch/-runter aus R2 hinaus).
