...
Noch offen: die eigentliche Claude-Code-Cloud-Routine-Anbindung (`routineBlueprintProvider` ist ein Platzhalter - erfordert einen echten, öffentlich erreichbaren Callback sowie `CLAUDE_ROUTINE_TRIGGER_URL`/`_TOKEN`, beides erst nach echtem Deployment sinnvoll einzurichten; die internen Routine-Endpunkte aus Abschnitt 7, `GET/POST /api/internal/course-generation/...`, sind entsprechend noch nicht gebaut); die dynamische Fragenmengen-Reduktion bei Überschreiten von `initialCostLimitUsd` (aktuell nur eine Statusmeldung, siehe Kommentar in `worker.ts::recordSpend`); der harte `HARD_MAX_OBJECTIVES`-Split großer Kurse in mehrere Versionen (aktuell bricht ein zu großer Blueprint einfach ab, statt automatisch aufgeteilt zu werden); Phase 8 (Härtung: drei echte Testkurse mit einem echten `ANTHROPIC_API_KEY`, Routine-/Rate-Limit-/Worker-Neustart-Simulation, Prompt-Injection-Testquellen, strukturierte Logs) - in dieser Sandbox ohne Live-API-Key nicht durchführbar, siehe Testplan im zugehörigen PR.
