...
**Phasen 2-7 sind in einem Rutsch umgesetzt (Nutzervorgabe: komplettes Dokument durchziehen, dann zum Testen melden).**

- **Phase 2 (Jobmodell und Statusmaschine):** `course_generation_jobs`/`course_generation_objectives` (Migration `0022_lowly_hedge_knight.sql`), Statuswerte 1:1 aus Abschnitt 6 übernommen plus ein ergänzter Zustand `package_ready` (der Katalog in Abschnitt 6 sieht keinen Zustand für "vollständig generiert und validiert, aber `autoPublish=false`" vor - terminal wie `published`, wartet auf manuelle Freigabe). `lib/server/course-generation/job-service.ts` kapselt Erzeugung (App-seitige Duplikaterkennung über den Titel, da vor dem Import anders als bei `content_generation_jobs` kein natürlicher DB-Schlüssel existiert), Statusübergänge und Objective-Fortschritt.
- **Phase 3 (PWA und Admin-API):** `POST/GET /api/admin/course-generation`, `GET/POST .../[id]`, `.../[id]/cancel`, `.../[id]/retry` (`app/api/admin/course-generation/`). Neues Panel `CourseGenerationPanel.tsx` im Admin-Dashboard (`/admin`, oberhalb des bisherigen manuellen "Kursstruktur anlegen"-Formulars, das unverändert erhalten bleibt) - Formular, Live-Fortschritt per Polling (2s, wie `ContentGenerationControl`), Kosten-/Limit-Anzeige, Abbrechen/Retry.
- **Phase 4 (Claude-Pro-Blueprint):** `BlueprintProvider`-Abstraktion mit zwei Implementierungen (`lib/server/course-generation/providers.ts`): `anthropicSonnetBlueprintProvider` (sofort nutzbar, nutzt den bestehenden, bereits getesteten `CURRICULUM_MODEL`-API-Pfad) und `routineBlueprintProvider` (Platzhalter, wirft absichtlich einen klaren Fehler statt still zu funktionieren, solange `CLAUDE_ROUTINE_TRIGGER_URL` nicht konfiguriert ist - siehe "Noch offen" unten). Aktiver Default: `anthropic-sonnet` - der in Abschnitt 12 beschriebene Kosteneinsparungs-Hebel (Sonnet über Claude-Pro-Kontingent statt bezahlter API) ist damit noch NICHT aktiv, siehe "Noch offen".
- **Phase 5 (Haiku-Worker mit Preflight/Canary/Batches/Circuit-Breaker):** `lib/server/course-generation/worker.ts::executeCourseGenerationJob()` - Gate A (`lib/server/course-generation/sources.ts::fetchSource()`, PDF via bestehendem `extractPdfPages()`, HTML via einfacher Tag-Entfernung), Gate B (`validateBlueprint()`), Kostenschätzung, Gate C (`pickCanaryIndex()` wählt bewusst das komplexeste/umfangreichste Objective zuerst), Gate D (progressive Batches `INITIAL_BATCH_SIZE`/`GENERATION_BATCH_SIZE`, Circuit Breaker bei Fehlerquote > `MAX_BATCH_ERROR_RATE` oder `MAX_IDENTICAL_ERRORS`). Läuft in-process (kein separater Worker-Container - siehe Abschnitt 0, dieselbe Begründung wie beim bestehenden `content_generation_jobs`-Muster für diese Größenordnung).
- **Phase 6 (Validierung und Reparatur):** `lib/server/course-generation/validation.ts` (deterministische Heuristiken: Fragenanzahl, Near-Duplicate-Erkennung, `requiredConcepts`-Abdeckung, Szenario-Pflicht bei `complexity: high`, Locator-Gültigkeit - bewusst OHNE die KI-Urteil erfordernden Heuristiken aus Abschnitt 10 wie "Distraktoren plausibel"). Zwei Reparaturstufen wie in Abschnitt 11 beschrieben: erst dasselbe (günstige) Modell bis `MAX_REPAIR_ATTEMPTS`, danach genau ein gesammelter Versuch mit `anthropicSonnetRepairProvider`.
- **Phase 7 (Veröffentlichung):** bei `autoPublish=true` läuft Import direkt in `executeCourseGenerationJob()`; bei `autoPublish=false` stoppt der Job bei `package_ready` und `POST .../retry` importiert das bereits validierte, gespeicherte `packageJson` gezielt (`publishPreparedCourseGenerationPackage()`), OHNE die Generierung erneut zu durchlaufen.
- **Resumability (Phase-2-Abnahme):** ein Objective, das bereits `status: "valid"` mit gespeichertem `contentJson` hat, wird bei einem erneuten Lauf auf demselben Job NICHT neu generiert (siehe Resume-Kurzschluss in `generateAndValidateObjective()`) - `retry` auf einem teilweise erfolgreichen Job (z. B. nach offenem Circuit Breaker) generiert nur die fehlgeschlagenen Objectives neu.
- **Kooperativer Abbruch:** `POST .../cancel` setzt den Status auf `cancelled`; der Worker prüft das vor jedem Batch nach (kein `AbortController` über die gesamte Aufrufkette, siehe `checkNotCancelled()`).
- **Test-/Fake-Provider:** `lib/server/course-generation/fake-providers.ts` + `npm run course:e2e -- <szenario>` (`happy-path`/`circuit-breaker`/`budget-exceeded`/`canary-failed`/`package-ready`) - alle fünf real gegen Postgres verifiziert (siehe PR), inklusive eines gezielten Resume-Tests (Circuit Breaker öffnet, dann Retry mit funktionierendem Provider - das bereits gültige Canary-Objective wird nicht neu generiert). Zusätzlich real über einen Browser gegen die laufende App getestet: Formular im Admin-Dashboard ausgefüllt, echter HTTP-Request, echter Hintergrund-Worker, echter (unerwarteter) Netzwerkfehler beim Quellenabruf sauber als `preflight_failed` abgefangen und in der UI mit Fehlermeldung + Retry/New-Job-Buttons angezeigt.

### Update 24. September 2026: Provider-Wechsel zurück zu Gemini + Phase-8-Härtung mit echten Testkursen

Der Nutzer hat den KI-Provider per PR #55 (eigener Merge, nicht Teil dieser Roadmap-Umsetzung) von der Anthropic Claude API zurück auf Gemini umgestellt, um bewusst ein anderes Modell zu testen. Die technischen Details in Abschnitt 0 oben (`getClaudeClient()`, `ANTHROPIC_API_KEY`, `CURRICULUM_MODEL`-Default `claude-sonnet-4-5`, `AnthropicSonnetProvider` etc.) sind dadurch veraltet - der Code läuft jetzt auf `lib/claude.ts`s `GoogleGenAI`-Client (`GEMINI_API_KEY`, Default-Modell `gemini-2.5-flash`), dieselbe `requestJson()`/`generateStructured()`-Abstraktion in `lib/ai/generate.ts` bedient aber weiterhin denselben Vertrag (Zod-Schema-Validierung, Retry-Loop, JSON-Reparatur). Die Kosten-Prämisse hinter dem "Claude-Pro-Routine"-Baustein (Abschnitt 3.2/3.3, kostenloses Kontingent statt bezahlter API) gilt für Gemini so nicht mehr 1:1 - das muss neu bewertet werden, bevor die Routine-Anbindung gebaut wird (siehe "Noch offen" unten).

Danach wurde der Teil von Phase 8 nachgeholt, der eine Sandbox ohne Live-Key nicht leisten konnte - **vier echte Testkurse gegen die echte Gemini API** (AEVO, LPIC-1, CKA, PCA), alle über den bestehenden manuellen Pfad (`content:draft-curriculum`/`content:draft-lessons`, nicht die neue One-Click-Blueprint-Pipeline). Dabei wurden vier reale Produktionsbugs gefunden und gefixt:

- `maxOutputTokens: 8192` zu knapp für vollständige Lesson+Fragen-Batches → auf das reale Modell-Maximum 65536 angehoben (PR #57/#58)
- alle 8 CLI-Scripts luden nur `.env.local`, nie `.env` (das docker-compose tatsächlich befüllt) → `.env` wird jetzt zuerst geladen (PR #59)
- `LESSONS_MODEL`-Default `"gemini-2.5-flash-lessons"` war kein existierendes Modell (404) → fällt jetzt wie `CURRICULUM_MODEL` auf ein reales Modell zurück (PR #60)
- `generateStructured()`s interne JSON-Korrektur hatte bei `gemini-3.6-flash` nur einen Versuch, der nicht immer reichte → auf zwei Korrekturversuche erhöht (PR #61)

CKA und PCA liefen im Anschluss beide vollständig durch ohne weitere Fehlermeldungen.

Noch offen: die eigentliche Claude-Code-Cloud-Routine-Anbindung (`routineBlueprintProvider` ist ein Platzhalter - erfordert einen echten, öffentlich erreichbaren Callback sowie `CLAUDE_ROUTINE_TRIGGER_URL`/`_TOKEN`; MUSS zudem neu bewertet werden, da die Kosten-Prämisse an Claude hing, siehe Update oben); die dynamische Fragenmengen-Reduktion bei Überschreiten von `initialCostLimitUsd` (aktuell nur eine Statusmeldung, siehe Kommentar in `worker.ts::recordSpend`); der harte `HARD_MAX_OBJECTIVES`-Split großer Kurse in mehrere Versionen (aktuell bricht ein zu großer Blueprint einfach ab, statt automatisch aufgeteilt zu werden - explizit noch NICHT über die neue One-Click-Pipeline getestet); der Rest von Phase 8 (Rate-Limit-/Worker-Neustart-Simulation, Prompt-Injection-Testquellen, strukturierte Logs).
